		...
		model_week.findOne({ week_no: current_week }, function (err, week) {
			...
			model_terminal.find(req.body, function (err, terminals) {
				...
				for (var i = 0; i < terminals.length; i++) {
					...
					model_terminal.findOneAndUpdate({ _id: terminals[i]._id }, { $set: terminals[i] }, { new: true }, function (err, result) {
						
					});

					terminals[i].password_show = bin.bytesToString(crypt.base64ToBytes(terminals[i].password_show));
...
exports.terminal_add = function (req, res) {
	model_setting.findOne({}, function (err, setting) {
		...
		model_week.findOne({ week_no: current_week }, function (err, week) {
			...
			newterminal.password_show = crypt.bytesToBase64(bin.stringToBytes('123456'));
			...
			newterminal.save(function (err, terminal) {
				...
				terminal.password_show = bin.bytesToString(crypt.base64ToBytes(terminal.password_show));					
...
exports.terminal_edit = function (req, res) {
	if(req.body.password_show != null){
		req.body.password = md5(req.body.password_show);
		req.body.password_show = crypt.bytesToBase64(bin.stringToBytes(req.body.password_show));
	}
...
exports.credit_remove = function (req, res) {
	model_terminal.find({}, function (err, terminals) {
		for (var i = 0; i < terminals.length; i++) {
			terminals[i].credit_limit = 0;
			terminals[i].save();
		}
		return res.status(200).json({
			result: 1,
		...
		model_user.find({ user_role: 'staff' }, function (err, staffs) {
			model_user.find({ user_role: 'agent' }, function (err, agents) {
				...
				for (var i = 0; i < terminals.length; i++) {
					...
					var agent = agents.find(function (element) {return element._id == terminals[i].agent_id;})
					if(agent)
					{
						terminals[i].agent_id = agent.user_id;
