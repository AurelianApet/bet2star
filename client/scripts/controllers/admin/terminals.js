					...
					.success(function (data, status, headers, config) {
						...
						if (data.result == 1) {
							...
							for(var i = 0; i < data.terminals.length; i++)
						  	{
						  		var agent = $scope.agents.find(function(element){ return element._id == data.terminals[i].agent_id; });
						  		if(agent)
						  			data.terminals[i].agent_id = agent.user_id;
						  	}
						...
						$scope.addTerminal = function () {
							...
							if (nEditing !== null) {
								...
								var data = $.param({
									...
									agent_id: $scope.agents[0] == undefined ? '' : $scope.agents[0]._id,
								});
								...
								$http.post(Server_api_url + 'terminal/terminal_add', data, UserInfo_service.http_config)
									.success(function (data, status, headers, config) {
										...
										if (data.result == 1) {
											...
											var agent_id = "";
											var user_agent = $scope.agents.find(function(element){ return element._id == data.terminal.agent_id; });
									  		if(user_agent)
									  			agent_id = user_agent.user_id;
											...
											nRow.childNodes[10].style = "text-align:center; width:300px;";
											nRow.childNodes[11].style = "text-align:center; width:360px";
						...
						function saveRow(oTable, nRow) {
							...
							var data = $.param({
								...
								agent_id: $('select')[0].selectedOptions[0].value,
								...
								password_show: jqInputs[4].value,
							});
										...
										for (var i = 0; i < terminals.length; i++) {
											for (var j = 0; j < terminals[i].options.length; j++) {
												if (terminals[i].options[j].current) {
													var commission = $('#cms_' + terminals[i]._id + '_' + j).val().trim() == "" ? 0 : parseFloat($('#cms_' + terminals[i]._id + '_' + j).val());
													$('#cms_' + terminals[i]._id + '_' + j).val(commission);
													terminals[i].options[j].commission = commission;
												}
											}
