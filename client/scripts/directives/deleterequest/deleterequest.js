	  ...
	  controller: function ($scope, UserInfo_service, $state, $interval, $location, $http, Server_api_url) {
		...
		$scope.timer = $interval(function () {
			...
			.success(function (data, status, headers, config) {
				if (data.result == 1) {
					$scope.filter.weekno = data.setting.current_week;
					$scope.allweek = [];
				    ...
				    var filterFunction = function (settings, data, dataIndex) {
						return parseInt(data[5], 10) === parseInt($scope.filter.weekno, 10);
					};
					$.fn.dataTable.ext.search[0] = filterFunction;
					betsTable.fnDraw();
